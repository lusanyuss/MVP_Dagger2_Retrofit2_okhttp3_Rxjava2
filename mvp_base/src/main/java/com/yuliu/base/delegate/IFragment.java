...
public interface IFragment {
     ...
     * 建议在有多个需要与外界交互的方法时, 统一传 { Message}, 通过 what 字段来区分不同的方法, 在 { #setData(Object)}
     * 方法中就可以 {@code switch} 做不同的操作, 这样就可以用统一的入口方法做多个不同的操作, 可以起到分发的作用
     * <p>
     * 调用此方法时请注意调用时 Fragment 的生命周期, 如果调用 { #setData(Object)} 方法时 { Fragment#onCreate(Bundle)} 还没执行
     * 但在 { #setData(Object)} 里却调用了 Presenter 的方法, 是会报空的, 因为 Dagger 注入是在 { Fragment#onCreate(Bundle)} 方法中执行的
     * 然后才创建的 Presenter, 如果要做一些初始化操作,可以不必让外部调用 { #setData(Object)}, 在 { #initData(Bundle)} 中初始化就可以了
     * <p>
     * Example usage:
     * <pre>
     ...
     * </pre>
     * <p>
     * { #setData(Object)} 框架是不会调用的, 是拿给开发者自己去调用的, 让 { Activity} 或者其他类可以和 { Fragment} 通信,
     * 并且因为 { #setData(Object)} 是 { IFragment} 的方法, 所以你可以通过多态, 持有父类,
     * 不持有具体子类的方式就可以和子类 { Fragment} 通信, 这样如果需要替换子类, 就不会影响到其他地方,
     * 并且 { #setData(Object)} 可以通过传入 { Message} 作为参数, 使外部统一调用 { #setData(Object)},
