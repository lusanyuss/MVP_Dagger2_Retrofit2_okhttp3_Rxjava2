...
public class LruCache<K, V> implements Cache<K, V> {
     ...
     *
     * @param size 这个缓存的最大 size,这个 size 所使用的单位必须和 { #getItemSize(Object)} 所使用的单位一致.
     */
     ...
     * 此时会回调 { #onItemEvicted(Object, Object)} 通知此方法当前被驱逐的条目
     *
    ...
    /**
     * 当缓存中已占用的总 size 大于所能允许的最大 size ,会使用  { #trimToSize(int)} 开始清除满足条件的条目
     */
    private void evict() {
        trimToSize(maxSize);
    }
