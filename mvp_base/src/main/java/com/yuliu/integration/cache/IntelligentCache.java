 ...
 */
package com.yuliu.integration.cache;
 ...
 * { IntelligentCache} 可根据您传入的 {@code key} 智能的判断您需要将数据存储至哪个存储容器, 从而针对数据
 * 的不同特性进行不同的存储优化
 * 调用 { IntelligentCache#put(Object, Object)} 方法, 使用 { #KEY_KEEP} + {@code key} 作为 key 传入的
 * {@code value} 可存储至 { #mMap} (数据永久存储至内存中, 适合比较重要的数据) 中, 否则储存至 { #mCache}
 * ================================================
 */
public class IntelligentCache<V> implements Cache<String, V> {
    ...
    /**
     * 将 { #mMap} 和 { #mCache} 的 {@code size} 相加后返回
     *
    ...
    /**
     * 将 { #mMap} 和 { #mCache} 的 {@code maxSize} 相加后返回
     *
    ...
    /**
     * 如果在 {@code key} 中使用 { #KEY_KEEP} 作为其前缀, 则操作 { #mMap}, 否则操作 { #mCache}
     *
     * @param key {@code key}
     * @return {@code value}
     */
    ...
    /**
     * 将 { #mMap} 和 { #mCache} 的 {@code keySet} 合并返回
     *
