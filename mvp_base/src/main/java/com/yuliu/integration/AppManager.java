...
import static com.yuliu.base.Platform.DEPENDENCY_SUPPORT_DESIGN;
...
public final class AppManager {
     ...
     * <p>
     * 提供给外部扩展 { AppManager} 的 { #onReceive(Message)} 方法
     */
     ...
     * 此方法作废, 现在可通过 { AppManager#getAppManager()} 直接访问 { AppManager}
     * <p>
     * 可通过 { #setHandleListener(HandleListener)}, 让外部可扩展新的事件
     *
     * @param message
     */
    @Deprecated
     ...
     * 建议在 { ConfigModule#injectAppLifecycle(Context, List)} 中
     * 通过 { AppLifecycles#onCreate(Application)} 在 App 初始化时,使用此方法传入自定义的 { HandleListener}
     *
     ...
     * 此方法作废, 现在可通过 { AppManager#getAppManager()} 直接访问 { AppManager}
     * <p>
     * 通过此方法远程遥控 { AppManager}, 使 { #onReceive(Message)} 执行对应方法
     *
     * @param msg { Message}
     */
    @Deprecated
    ...
    /**
     * 将在前台的 { Activity} 赋值给 {@code currentActivity}, 注意此方法是在 { Activity#onResume} 方法执行时将栈顶的 { Activity} 赋值给 {@code currentActivity}
     * 所以在栈顶的 { Activity} 执行 { Activity#onCreate} 方法时使用 { #getCurrentActivity()} 获取的就不是当前栈顶的 { Activity}, 可能是上一个 { Activity}
     * 如果在 App 启动第一个 { Activity} 执行 { Activity#onCreate} 方法时使用 { #getCurrentActivity()} 则会出现返回为 {@code null} 的情况
     ...
     * 这时调用 { #getCurrentActivity()} 有可能返回 {@code null}, 所以请注意使用场景和 { #getTopActivity()} 不一样
     * <p>
     * Example usage:
     ...
     * 如果返回为 {@code null}, 说明没有前台 { Activity} (用户返回桌面或者打开了其他 App 会出现此状况), 则不做任何操作, 不为 {@code null}, 则弹出 { Dialog}
     *
     * @return
     */
    @Nullable
     ...
     * 还是会返回这个最近启动的 { Activity}, 因此基本不会出现 {@code null} 的情况
