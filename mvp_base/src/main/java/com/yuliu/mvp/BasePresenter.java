...
import com.yuliu.integration.EventBusManager;
...
public class BasePresenter<M extends IModel, V extends IView> implements IPresenter, LifecycleObserver {
    ...
    /**
     * 只有当 {@code mRootView} 不为 null, 并且 {@code mRootView} 实现了 { LifecycleOwner} 时, 此方法才会被调用
     * 所以当您想在 { Service} 以及一些自定义 { View} 或自定义类中使用 {@code Presenter} 时
    ...
    void onDestroy(LifecycleOwner owner) {
        /**
         * 注意, 如果在这里调用了 { #onDestroy()} 方法, 会出现某些地方引用 {@code mModel} 或 {@code mRootView} 为 null 的情况
         * 比如在 { RxLifecycle} 终止 { Observable} 时, 在 { io.reactivex.Observable#doFinally(Action)} 中却引用了 {@code mRootView} 做一些释放资源的操作, 此时会空指针
